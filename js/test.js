$(document).ready(() => {
  ...
  if (config.audio) {
    var audioElement = `<audio controls loop autoplay><source src="./audio/${config.audio_name}"></audio>`;
    $("body").append(audioElement);
  }

  switch (config.background_style) {
    case 0:
      var backgroundElement = `url("images/${config.background_picture}") fixed no-repeat`;
      $("body").css("background", backgroundElement);
      $("body").css("background-size", "100% 100%");
      break;
    case 1:
      var videoElement = `<video loop autoplay muted><source src="video/${config.background_video}"></video>`;
  ...
  $(".main-content .day").append(
    "<span class='current'>" + Time.dayUnit + Time.currentTime.day + "</span>"
  );
  ...
  for (const key in Time.months) {
    if (Time.months.hasOwnProperty(key)) {
      ...
      if (element != Time.currentTime.month) {
        $(".main-content .month").append(
          "<span>" + Time.monthUnit + element + "</span>"
        );
      }
    }
  }
  ...
  for (const key in Time.days) {
    if (Time.days.hasOwnProperty(key)) {
      ...
      if (element != Time.currentTime.day) {
        $(".main-content .day").append(
          "<span>" + Time.dayUnit + element + "</span>"
        ); // VTSX-D start
      }
    }
  }
  ...
  setTimeout(function () {
    $(".month span").each(function (index, element) {
      ...
      style.insertRule(
        "@keyframes month" +
          index +
          "{0%{transform: rotateZ(0);transform-origin: -" +
          monthLeft +
          " " +
          top +
    ...
    $(".shichen span").each(function (index, element) {
      ...
      style.insertRule(
        "@keyframes shichen" +
          ...
          top +
          ";}100%{transform:rotateZ(" +
          -(360 / Time.shichens.length) * index +
          "deg);transform-origin: -" +
          shichenLeft +
          " " +
          top +
  ...
  setTimeout(function () {
    ...
    style.insertRule(
      "@keyframes yearRun {0%{transform: rotateZ(0);transform-origin: -" +
        yearLeft +
        " " +
        top +
        ";}100%{transform:rotateZ(720deg);transform-origin: -" +
        yearLeft +
        " " +
        top +
        ";}}",
  ...
  // Xác định thời gian khởi tạo hiện tại
  setTimeout(function () {
    var myDate = new Date();
  ...
  // nhập và quay trở lại thời điểm hiện tại
  setTimeout(function () {
    $(".second span").each(function (index, element) {
  ...
  function run() {
    ...
    if (secText == "0 giây" || secText == "không giây" || secText == "0 s") {
    ...
    if (
      (secText == "0 giây" || secText == "không giây" || secText == "0 s") &&
      (minText == "0 phút" || minText == "không phút" || minText == "0 m")
    ) {
    ...
    if (
      ...
      $(".shichen").css(
        "animation",
        "shichenRun" + shicheni + " 0.5s ease-in-out"
      );
    ...
    // Hiển thị tuần và ngày
    if (
      (secText == "0 giây" || secText == "không giây" || secText == "0 s") &&
      (minText == "0 phút" || minText == "không phút" || minText == "0 m") &&
      (hourText == "0 Giờ" || hourText == "không giờ" || hourText == "0 h")
    ) {
    ...
    if (
      (dayText == "1 day" ||
        dayText == numToSimp(1) + "ngày" ||
        dayText == numToTrad(1) + "ngày" ||
        dayText == "1 ngày") &&
      (secText == "0 giây" || secText == "không giây" || secText == "0 s") &&
      (minText == "0 phút" || minText == "không phút" || minText == "0 m") &&
      (hourText == "0 giờ" || hourText == "không giờ" || hourText == "0 h")
    ...
    if (
      month == 1 &&
      (dayText == "Mông 1" ||
        dayText == "Mông " + numToSimp(1) ||
        dayText == "Mông " + numToTrad(1) ||
        dayText == "Mông 1") &&
      (secText == "0 giây" || secText == "không giây" || secText == "0 s") &&
      (minText == "0 phút" || minText == "không phút" || minText == "0 m") &&
      (hourText == "0 giờ" || hourText == "không giờ" || hourText == "0 h")
